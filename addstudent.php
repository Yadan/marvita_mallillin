...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($studentnumber) || empty($ffname)||  empty($mname)||  empty($lname)|| empty($gender) || empty($address)) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            $user_sql = "INSERT INTO users (studentnumber, ffname,mname,lname,fname, gender, address) VALUES ('$studentnumber', '$ffname','$mname','$lname','$fname', '$gender', '$address')";
            $enrollment_sql = "INSERT INTO enrollments (studentnumber, fname) VALUES ('$studentnumber', '$fname')";

            $user_result = $conn->query($user_sql);
            $enrollment_result = $conn->query($enrollment_sql);

            if (!$user_result || !$enrollment_result) {
